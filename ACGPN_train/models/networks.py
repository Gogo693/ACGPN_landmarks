...
class UnetMask(nn.Module):
    ...
    def forward(self, input, refer, mask, lm_c_map, cloth_rep):
        ...
        input, warped_mask, warped_cloth_lm, rx, ry, cx, cy, rg,cg = self.stn(input, torch.cat([mask, cloth_rep, input], 1), mask, lm_c_map)
...
class CNN(nn.Module):
    def __init__(self, num_output, input_nc=5, ngf=8, n_layers=5, norm_layer=nn.InstanceNorm2d, use_dropout=False):
        ...
        downconv = nn.Conv2d(13, ngf, kernel_size=4, stride=2, padding=1)
